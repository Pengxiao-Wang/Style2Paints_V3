...
class Inception(nn.Module):
    def __init__(self, in_channel, br_1x1, br_3x3_reduce, br_3x3,
        ...
        self.add_module('pool', nn.MaxPool2d(kernel_size=3, stride=1, padding=1,ceil_mode=False))
...
class GoogleNet(nn.Module):
    def __init__(self):
        ...
        self.add_module('conv1_7x7_s2', nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3))
        self.add_module('conv1_relu_7x7', nn.ReLU())

        self.add_module('pool1', nn.MaxPool2d(3, stride=2, padding=1, ceil_mode=False))

        self.add_module('conv2_3x3_reduce', nn.Conv2d(64, 64, kernel_size=1))
        self.add_module('conv2_relu_3x3_reduce', nn.ReLU())
        self.add_module('conv2_3x3', nn.Conv2d(64, 192, kernel_size=3, padding=1))
        self.add_module('conv2_relu_3x3', nn.ReLU())

        self.add_module('pool2', nn.MaxPool2d(3, stride=2, padding=1, ceil_mode=False))

        self.inception_3a = Inception(192, 64, 96, 128, 16, 32, 32)
        self.inception_3b = Inception(256, 128, 128, 192, 32, 96, 64)

        self.add_module('pool3', nn.MaxPool2d(3, stride=2, padding=1, ceil_mode=False))
        ...
        self.add_module('pool4', nn.MaxPool2d(3, stride=2, padding=1, ceil_mode=False))
        ...
        self.add_module('loss3_classifier', nn.Linear(1024, 1000))

    def forward(self, x):
        #import pdb;pdb.set_trace()
        x = getattr(self, 'conv1_7x7_s2')(x)
        x = getattr(self, 'conv1_relu_7x7')(x)
        x = self.pool1(x)#[2, 64, 56, 56]
        x = local_response_norm(x, 5)

        x = getattr(self, 'conv2_3x3_reduce')(x)
        x = getattr(self, 'conv2_relu_3x3_reduce')(x)
        x = getattr(self, 'conv2_3x3')(x)
        x = getattr(self, 'conv2_relu_3x3')(x)
        x = local_response_norm(x, 5)
        x = self.pool2(x)#[2, 192, 28, 28]

        x = self.inception_3a(x)
        x = self.inception_3b(x)
        x = self.pool3(x)#[2, 480, 14, 14]
        ...
        x = self.pool4(x)#[2, 832, 7, 7]

        x = self.inception_5a(x)
        x = self.inception_5b(x)
        x = self.pool5(x)#[2, 1024, 1, 1]
        x = self.drop(x)#[2, 1024, 1, 1]

        # x = x.view(-1, 1024)
        # x = getattr(self, 'loss3_classifier')(x)

        return x


def get_googlenet(pretrain=False, pth_path='./weights/google_net.pth'):
    model = GoogleNet()
    if pretrain:
